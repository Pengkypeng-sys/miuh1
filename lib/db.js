...
// Ambil SEMUA baris tabel log_aktivitas, nembus batas "Max Rows" 1000 punya Supabase project
// (batas itu di server, gak bisa ditembus cuma pake .limit() client) — loop per 1000 baris pake .range().
export async function fetchAllLogAktivitas(selectCols) {
  const hasil = [];
  let from = 0;
  const PAGE = 1000;
  while (true) {
    const rows = throwIfError(await db().from('log_aktivitas').select(selectCols).range(from, from + PAGE - 1));
    hasil.push(...rows);
