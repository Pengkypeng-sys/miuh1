...
// Ambil SEMUA baris 1 tabel, nembus batas "Max Rows" 1000 punya Supabase project (batas itu di
// server, gak bisa ditembus cuma pake .limit() client) — loop per 1000 baris pake .range().
// `buildQuery` opsional buat nambahin .eq()/.in() dll sebelum di-range.
export async function fetchAllRows(table, selectCols, buildQuery) {
  const hasil = [];
  let from = 0;
  const PAGE = 1000;
  while (true) {
    ...
    const rows = throwIfError(await q.range(from, from + PAGE - 1));
    hasil.push(...rows);
...
export const fetchAllLogAktivitas = (selectCols) => fetchAllRows('log_aktivitas', selectCols);
