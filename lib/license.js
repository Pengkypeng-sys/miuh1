import { getValues, setValues, getSheetTitles } from './sheets';
...
export async function getLicenseExpiry() {
  const sheets = await getSheetTitles();
  if (!sheets.includes('Lisensi')) {
    await createLisensiSheet(DEFAULT_EXPIRY);
    return DEFAULT_EXPIRY;
  }
  const rows = await getValues('Lisensi!A2:B');
...
async function createLisensiSheet(expiry) {
  ...
  await setValues('Lisensi!A1:B2', [['Key', 'Value'], ['LICENSE_EXPIRY', expiry]], true);
}
...
export async function setLicenseExpiry(newExpiry) {
  ...
  await setValues(`Lisensi!A${rowNum}:B${rowNum}`, [["LICENSE_EXPIRY", newExpiry]], true);
}
